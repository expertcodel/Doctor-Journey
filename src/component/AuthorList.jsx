...
export default function AuthorList({ authorList, totalItems }) {
    ...
    const [authorLists, setauthorLists] = useState(authorList);
    ...
    const deleteRecords = async (authorId) => {
        ...
        if (!res.status) {
            ...
            sessionStorage.setItem('successMsg', 'Author Deleted Successfully');
            // setPopup(false);
            // router.push(`/${userData.usertype}/authors/list`)
            window.location.href = `/${userData.usertype}/authors/list`
                    ...
                    <div className="row">
                        <div className="col-12">
                            <div className="page-title-box d-sm-flex align-items-center justify-content-between bg-galaxy-transparent">
                                <h4 className="mb-sm-0">Authors List</h4>
                    ...
                    <div className="row"><div className="col-lg-12"><div className="card" id="invoiceList">
                        ...
                        <div className="card-body"><form><div className="row g-4 mb-3">
                            <div className="col-sm-auto">
                                <div>
                                    <Link href={`/${userData.usertype}/authors/create`} className="btn btn-success"><i className="ri-add-line align-bottom me-1" /> Add New</Link>
                                </div>
                            </div>
                        ...
                        <div className="card-body"><div><div className="table-responsive table-card"><table className="table align-middle table-nowrap" id="invoiceTable"><thead className="text-muted"><tr><th className=" text-uppercase" data-sort="invoice_id">authors Id</th><th className="text-uppercase" data-sort="customer_name">Profile</th><th className=" text-uppercase" data-sort="email">Specialization</th><th className=" text-uppercase" data-sort="email">Qualification</th><th className=" text-uppercase" data-sort="country">Status</th><th className=" text-uppercase" data-sort="action">Action</th></tr></thead>

                            {authorLists.length > 0 && authorLists.map((item, i) => <tbody key={item.authorId} className="list form-check-all"><tr><td className="id"><a href="javascript:throw new Error('React has blocked a javascript: URL as a security precaution.')" data-id={25000351} className="fw-medium link-primary">{item.authorId} </a></td><td className="customer_name"><div className="d-flex align-items-center"><img className="avatar-xs rounded-circle me-2" src={item.profileImage} alt='not found' />{item.authorName} </div></td><td className="email">{item.specialization}</td><td className="email">{item.qualification}</td><td className="status"><span className={item.status ? "badge bg-success-subtle text-success text-uppercase" : "badge bg-warning-subtle text-warning text-uppercase"}>{item.status ? 'active' : 'inactive'}</span></td>
                                <td>
                                ...
                                {button > 0 && <div className="pagination-wrap hstack gap-2" style={{ "display": "flex" }}>
                                    ...
                                    <ul className="pagination listjs-pagination mb-0">
                                        ...
                                        {Array.from({ length: button }, (_, i) => <li className="active" key={i}><button className="page" key={i} onClick={() => pagination(i + 1)} style={{ backgroundColor: idx === i + 1 && '#ff681a', border: 'none' }}>{i + 1}</button>
                                        </li>)
                                        }
