...
export default function VideoList({ videoList, totalItems, usertype, userId }) {
    ...
    const [videoLists, setvideoLists] = useState(videoList);
    ...
    const searching = async (idx, name) => {
        ...
        const response = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/superAdmin/doctors/uploadVideo/?page=${1}&name=${name}&userId=${userId}&usertype=${typeof (usertype)}`);
        setName(name);
        setIdx(1);
        const res = await response.json();
        if (res.status) {
    ...
    //     formData.append('data', JSON.stringify({ videoId }));
    //     for (let i = 0; i < file.length; i++) {
    //         formData.append('file', file[i]);
    //     }

    //     const response = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/superAdmin/videos/uploadImages`, { method: 'POST', body: formData });
    //     const res = await response.json();
    //     if (res.status) {
            ...
            <div className="page-content">
                <div className="container-fluid">
                    ...
                    <div className="row"><div className="col-lg-12"><div className="card" id="invoiceList">
                        ...
                        </table> </div> </div><div className="noresult" style={{ display: button > 0 && "none" }}><div className="text-center"><lord-icon src="https://cdn.lordicon.com/msoeawqm.json" trigger="loop" colors="primary:#121331,secondary:#08a88a" style={{ "width": "75px", "height": "75px" }} /><h5 className="mt-2">Sorry! No Result Found</h5></div></div>

                            <div className="d-flex justify-content-end mt-3">
                                <div className="noresult" style={{ display: button > 0 && "none" }}><div className="text-center"><lord-icon src="https://cdn.lordicon.com/msoeawqm.json" trigger="loop" colors="primary:#121331,secondary:#08a88a" style={{ "width": "75px", "height": "75px" }} /><h5 className="mt-2">Sorry! No Result Found</h5></div></div></div>
