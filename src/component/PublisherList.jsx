...
export default function PublisherList({ publisherList, totalItems }) {
    ...
    const [publisherLists, setpublisherLists] = useState(publisherList);
    ...
    const deleteRecords = async (publisherId) => {
        ...
        if (!res.status) {
            ...
            sessionStorage.setItem('successMsg', 'Publisher Deleted Successfully');
            // setPopup(false);
            // router.push(`/${userData.usertype}/publisher/list`)
            window.location.href = `/${userData.usertype}/publisher/list`
                    ...
                    <div className="row">
                        <div className="col-12">
                            <div className="page-title-box d-sm-flex align-items-center justify-content-between bg-galaxy-transparent">
                                <h4 className="mb-sm-0">Publisher List</h4>
                    ...
                    <div className="row"><div className="col-lg-12"><div className="card" id="invoiceList">
                        ...
                        <div className="card-body"><form><div className="row g-4 mb-3">
                            <div className="col-sm-auto">
                                <div>
                                    <Link href={`/${userData.usertype}/publisher/create`} className="btn btn-success"><i className="ri-add-line align-bottom me-1" /> Add New</Link>
                                </div>
                            </div>
                        ...
                        <div className="card-body"><div><div className="table-responsive table-card"><table className="table align-middle table-nowrap" id="invoiceTable"><thead className="text-muted"><tr><th className=" text-uppercase" data-sort="invoice_id">publisher Id</th><th className="text-uppercase" data-sort="customer_name">Profile</th><th className=" text-uppercase" data-sort="email">Specialization</th><th className=" text-uppercase" data-sort="email">Qualification</th><th className=" text-uppercase" data-sort="country">Status</th><th className=" text-uppercase" data-sort="action">Action</th></tr></thead>

                            {publisherLists.length > 0 && publisherLists.map((item, i) => <tbody key={item.publisherId} className="list form-check-all"><tr><td className="id"><a href="javascript:throw new Error('React has blocked a javascript: URL as a security precaution.')" data-id={25000351} className="fw-medium link-primary">{item.publisherId} </a></td><td className="customer_name"><div className="d-flex align-items-center"><img className="avatar-xs rounded-circle me-2" src={item.profileImage} alt='not found' />{item.publisherName} </div></td><td className="email">{item.specialization}</td><td className="email">{item.qualification}</td><td className="status"><span className={item.status ? "badge bg-success-subtle text-success text-uppercase" : "badge bg-warning-subtle text-warning text-uppercase"}>{item.status ? 'active' : 'inactive'}</span></td>
                                <td>
                                    <div class="d-flex gap-2">
                                        <div class="edit">
                                            <Link class="btn btn-sm btn-success edit-item-btn" href={`/${userData.usertype}/publisher/update/${item.publisherId}`}>Edit</Link>
                                        </div>
                                        <div class="remove">
                                            <button class="btn btn-sm btn-danger remove-item-btn" onClick={() => openPopup(item.publisherId)}>Remove</button>
                                        </div>
                                    </div>
                                </td></tr>
