...
export default function OrganizationList({ organizationList, totalItems }) {
    ...
    const [organizationLists, setorganizationLists] = useState(organizationList);
    ...
    const deleteRecords = async (organizationId) => {

        const response = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/superAdmin/organization`, {
            method: "DELETE",
        ...
        if (!res.status) {
            ...
            sessionStorage.setItem('successMsg', 'Organization Deleted Successfully');
            // setPopup(false);
            // router.push(`/${userData.usertype}/organization/list`)
            window.location.href = `/${userData.usertype}/organization/list`
    ...
    const searching = async (idx, name) => {
        ...
        const response = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/superAdmin/organization/?page=${1}&name=${name}`);
        setName(name);
setIdx(1);
        const res = await response.json();
        if (res.status) {
            setorganizationLists(res.organizationlist);
    ...
    const pagination = async (idx) => {
        ...
        if (idx > 0 && idx <= button) {
            const response = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/superAdmin/organization/?page=${idx}&name=${name}`);
            setIdx(idx);
            const res = await response.json();
            if (res.status) {
                setorganizationLists(res.organizationlist);
                    ...
                    <div className="row">
                        <div className="col-12">
                            <div className="page-title-box d-sm-flex align-items-center justify-content-between bg-galaxy-transparent">
                                <h4 className="mb-sm-0">Organization List</h4>
                    ...
                    <div className="row"><div className="col-lg-12"><div className="card" id="invoiceList">
                        ...
                        <div className="card-body"><form><div className="row g-4 mb-3">
                            <div className="col-sm-auto">
                                <div>
                                    <Link href={`/${userData.usertype}/organization/create`} className="btn btn-success"><i className="ri-add-line align-bottom me-1" /> Add New</Link>
                                </div>
                            </div>
                        ...
                        <div className="card-body"><div><div className="table-responsive table-card"><table className="table align-middle table-nowrap" id="invoiceTable"><thead className="text-muted"><tr><th className=" text-uppercase" data-sort="invoice_id">organization Id</th><th className="text-uppercase" data-sort="customer_name">Profile</th><th className=" text-uppercase" data-sort="email">Specialization</th><th className=" text-uppercase" data-sort="email">Qualification</th><th className=" text-uppercase" data-sort="country">Status</th><th className=" text-uppercase" data-sort="action">Action</th></tr></thead>

                            {organizationLists.length > 0 && organizationLists.map((item, i) => <tbody key={item.organizationId} className="list form-check-all"><tr><td className="id"><a href="javascript:throw new Error('React has blocked a javascript: URL as a security precaution.')" data-id={25000351} className="fw-medium link-primary">{item.organizationId} </a></td><td className="customer_name"><div className="d-flex align-items-center"><img className="avatar-xs rounded-circle me-2" src={item.profileImage} alt='not found' />{item.organizationName} </div></td><td className="email">{item.specialization}</td><td className="email">{item.qualification}</td><td className="status"><span className={item.status ? "badge bg-success-subtle text-success text-uppercase" : "badge bg-warning-subtle text-warning text-uppercase"}>{item.status ? 'active' : 'inactive'}</span></td>
                                <td>
