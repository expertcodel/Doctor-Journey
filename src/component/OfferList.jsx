...
export default function OfferList({ offerList, totalItems }) {
    ...
    const deleteRecords = async (offerId) => {
    ...
    const searching = async (idx, name) => {
        ...
        const response = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/superAdmin/offer/?page=${1}&name=${name}`);
        setName(name);
setIdx(1);
        const res = await response.json();
        if (res.status) {
            setofferLists(res.offerlist);
    ...
    const pagination = async (idx) => {
        ...
        if (idx > 0 && idx <= button) {
            const response = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/superAdmin/offer?page=${idx}&name=${name}`);
            setIdx(idx);
            const res = await response.json();
            if (res.status) {
                setofferLists(res.offerlist);
            ...
            <div className="page-content">
                <div className="container-fluid">
                    ...
                    <div className="row">
                        <div className="col-12">
                            <div className="page-title-box d-sm-flex align-items-center justify-content-between bg-galaxy-transparent">
                                <h4 className="mb-sm-0">Offers List</h4>
                    ...
                    <div className="row"><div className="col-lg-12"><div className="card" id="invoiceList">
                        ...
                        <div className="card-body"><div><div className="table-responsive table-card"><table className="table align-middle table-nowrap" id="invoiceTable"><thead className="text-muted"><tr><th className=" text-uppercase" data-sort="invoice_id">ID</th><th className="text-uppercase" data-sort="customer_name">Discount</th><th className=" text-uppercase" data-sort="action">Action</th></tr></thead>

                            {offerLists.length > 0 && offerLists.map((item, i) => <tbody key={item.offerId} className="list form-check-all"><tr><td className="id"><a href="javascript:throw new Error('React has blocked a javascript: URL as a security precaution.')" data-id={25000351} className="fw-medium link-primary">{item.offerId} </a></td><td className="customer_name"><div className="d-flex align-items-center"><img className="avatar-xs rounded-circle me-2" src={item.offerImage} alt='not found' />{item.offerDiscount} </div></td>
                                <td>
                                    <div class="d-flex gap-2">
                                        <div class="edit">
                                            <Link class="btn btn-sm btn-success edit-item-btn" href={`/dashboard/journal/offer/update/${item.offerId}`}>Edit</Link>
                                        </div>
                                        <div class="remove">
                                            <button class="btn btn-sm btn-danger remove-item-btn deleteBtn" onClick={() => openPopup(item.offerId)}><i className='ri-delete-bin-line'></i></button>
                                        </div>
                                    </div>
                                </td></tr>
