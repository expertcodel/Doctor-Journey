...
export default function Testimonials({ testimonialList, totalItems }) {
    ...
    const [testimonialLists, setTestimonialLists] = useState(testimonialList);
    ...
    const deleteRecords = async (testimonialId) => {
            ...
            <div className="page-content">
                <div className="container-fluid">
                    ...
                    <div className="row">
                        <div className="col-12">
                            <div className="page-title-box d-sm-flex align-items-center justify-content-between bg-galaxy-transparent">
                                <h4 className="mb-sm-0">Testimonials List</h4>
                    ...
                    <div className="row"><div className="col-lg-12"><div className="card" id="invoiceList">
                        ...
                        <div className="card-body"><div><div className="table-responsive table-card"><table className="table align-middle table-nowrap" id="invoiceTable"><thead className="text-muted"><tr><th className=" text-uppercase" data-sort="invoice_id">ID</th><th className="text-uppercase" data-sort="customer_name">Name</th><th className=" text-uppercase" data-sort="country">Designation</th><th className=" text-uppercase" data-sort="status">Status</th><th className=" text-uppercase" data-sort="action">Action</th></tr></thead>

                            {testimonialLists.length > 0 && testimonialLists.map((item, i) => <tbody key={item.testimonialId} className="list form-check-all"><tr><td className="id"><a href="javascript:throw new Error('React has blocked a javascript: URL as a security precaution.')" data-id={25000351} className="fw-medium link-primary">{item.testimonialId} </a></td><td className="customer_name"><div className="d-flex align-items-center"><img className="avatar-xs rounded-circle me-2" src={item.image} alt='not found' />{item.name} </div></td><td className="country">{item.designation}</td><td className="status"><span className={item.status ? "badge bg-success-subtle text-success text-uppercase" : "badge bg-warning-subtle text-warning text-uppercase"}>{item.status ? 'active' : 'inactive'}</span></td>
                                <td>
