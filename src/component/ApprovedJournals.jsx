...
function ApprovedJournals({ journalLists, totalItems, usertype, userId }) {
    ...
    const searching = async (idx, name) => {
        ...
        const response = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/superAdmin/getJournal/?page=${1}&name=${name}&userId=${userId}&usertype=${typeof (usertype)}`);
        setName(name);
setIdx(1);
    ...
    const pagination = async (idx) => {
        ...
        if (idx > 0 && idx <= button) {
            const response = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/superAdmin/getJournal/?page=${idx}&name=${name}&userId=${userId}&usertype=${typeof (usertype)}`);
            setIdx(idx);
            const res = await response.json();
            if (res.status) {
                setJournallist(res.journallist);
    ...
    const deleteMultipleRecords = async () => {
        ...
        if (!res.status) {
            ...
            window.location.href = "/dashboard/journal/approvedjournals"
        }
                                    ...
                                    <div className="card-body">
                                      ...
                                      <button type="button" className="btn btn-sm btn-light" onClick={() => handleJournal(item)}>
                                        Details
                                                                        ...
                                                                        <span>
                                                                            <Link href={`/dashboard/publishjournal/${item.journalsId}`} className="btn btn-sm btn-light">
                                                                                Publish
                                                                            </Link>
                                                                        </span>
                                                                    </td>
