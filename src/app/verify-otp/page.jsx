...
export default function VerifyOtp() {
    ...
    const [sessionExist, setSessionexist] = useState(sessionStorage.getItem('data') ? JSON.parse(sessionStorage.getItem('data')) : null)

    // Load email and phone from localStorage
    useEffect(() => {

        //  const userregistration=sessionStorage.getItem('data');
        if (!sessionExist) {
    ...
    const handleResendEmailOtp = async () => {
        ...
        await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/otpCreate`, {
            method: "POST",
            headers: { "Content-Type": "application/json", 'api_key': process.env.NEXT_PUBLIC_SECRET_KEY },
            body: JSON.stringify({ email: sessionExist.email,name:sessionExist.name }),
        });
    };
    ...
    const handleVerify = async (e) => {
        ...
        const response = await fetch("/api/otpVerify", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ data: sessionExist, otp: payload.emailOtp }),
        });
