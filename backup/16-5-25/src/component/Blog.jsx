...
export default function Blog({ buttons, blogList, totalPages }) {
  ...
  const deleteRecords = async (blogId) => {
    ...
    const response = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/superAdmin/blogs`, {
      method: "DELETE",
  ...
  const pagination = async (idx) => {
    ...
    if (idx > 0 && idx <= buttons) {
      ...
      setBlogidx(idx);
      const response = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/superAdmin/blogs/?page=${idx}&name=${name}`, {
        method: 'GET',
