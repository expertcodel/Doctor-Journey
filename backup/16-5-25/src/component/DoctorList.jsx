...
export default function DoctorList({ doctorList, totalItems }) {
    ...
    const deleteRecords = async (doctorId) => {
    ...
    const searching = async (idx, name) => {
        ...
        const response = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/superAdmin/doctors/?page=${idx}&name=${name}`);
        setName(name);
    ...
    const uploadImages = async (e) => {
        ...
        formData.append('data', JSON.stringify({ doctorId }));
        for (let i = 0; i < file.length; i++) {
            formData.append('file', file[i]);
        }

        const response = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/superAdmin/doctors/uploadImages`, { method: 'POST', body: formData });
        const res = await response.json();
        if (res.status) {
            ...
            <div className="page-content">
                <div className="container-fluid">
                    ...
                    <div className="row">
                        <div className="col-12">
                            <div className="page-title-box d-sm-flex align-items-center justify-content-between bg-galaxy-transparent">
                                <h4 className="mb-sm-0">Doctors List</h4>
                    ...
                    <div className="row"><div className="col-lg-12"><div className="card" id="invoiceList">
                        ...
                        <div className="card-body"><div><div className="table-responsive table-card"><table className="table align-middle table-nowrap" id="invoiceTable"><thead className="text-muted"><tr><th className=" text-uppercase" data-sort="invoice_id">Doctors Id</th><th className="text-uppercase" data-sort="customer_name">Profile</th><th className=" text-uppercase" data-sort="email">Specialization</th><th className=" text-uppercase" data-sort="email">Qualification</th><th className=" text-uppercase" data-sort="country">Status</th><th className=" text-uppercase" data-sort="action">Action</th><th className=" text-uppercase" data-sort="action">Gallery</th></tr></thead>

                            {doctorLists.length > 0 && doctorLists.map((item, i) => <tbody key={item.doctorId} className="list form-check-all"><tr><td className="id"><a href="javascript:throw new Error('React has blocked a javascript: URL as a security precaution.')" data-id={25000351} className="fw-medium link-primary">{item.doctorId} </a></td><td className="customer_name"><div className="d-flex align-items-center"><img className="avatar-xs rounded-circle me-2" src={item.profileImage} alt='not found' />{item.doctorName} </div></td><td className="email">{item.specialization}</td><td className="email">{item.qualification}</td><td className="status"><span className={item.status ? "badge bg-success-subtle text-success text-uppercase" : "badge bg-warning-subtle text-warning text-uppercase"}>{item.status ? 'active' : 'inactive'}</span></td>
                                <td>
                                    ...
                                    <div class="d-flex gap-2">
                                        ...
                                        <div class="remove">
                                            <button class="btn btn-sm btn-danger remove-item-btn" data-bs-toggle="modal"
                                                data-bs-target="#deletetable" onClick={() => setDoctorid(item.doctorId)}>Upload</button>
                                        </div>
                                    </div>
                                </td>
                ...
                >
                    <div className="modal-dialog modal-dialog-centered">
                        <div className="modal-content">
                            ...
                            <div className="modal-body">
                                <form onSubmit={uploadImages} className='row g-3'>
                                    <div className="col-12">
                                        <label className="">
                                    ...
                                    <div className="d-flex col-12 justify-content-center">
                                        <button
                                            type="button"
                                            className="btn w-sm btn-light me-3"
                                            data-bs-dismiss="modal"
                                        >
                                            Close
                                        </button>
